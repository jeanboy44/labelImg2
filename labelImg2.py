...
from libs.combobox2 import ComboBox2
...
from libs.renameLabelDialog import RenameLabelDialog
from libs.exportFilesDialog import ExportFilesDialog
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(
        ...
        # Create file list layout
        file_list_layout = QVBoxLayout()
        file_list_layout.setContentsMargins(0, 0, 0, 0)

        # Create and add a widget for refresh button for label - image dictionary
        # self.file_list_refresh_button = QToolButton()
        # self.file_list_refresh_button.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)

        # Create and add a widget for double clicked item
        self.file_list_widget = QListWidget()
        self.file_list_widget.itemDoubleClicked.connect(self.file_item_double_clicked)

        # # # Create and add a widget for filtering file lists
        # self.all_label_combo_box = QComboBox()
        # self.all_label_combo_box.currentIndexChanged(self.update_all_label_combo_box)

        self.all_label_combo_box_refresh_btn = QPushButton('refresh')
        self.all_label_combo_box_refresh_btn.clicked.connect(self.update_all_label_combo_box)
        self.all_label_combo_box = ComboBox2(self)

        # Create and add a widget for showing file lists
        # file_list_layout.addWidget(self.file_list_refresh_button)
        file_list_layout.addWidget(self.all_label_combo_box_refresh_btn)
        file_list_layout.addWidget(self.all_label_combo_box)
        ...
        show_rename_dialog = action(
            get_str("renameLabel"),
            self.show_rename_label_dialog,
            None,
            "tools",
            get_str("renameLabel"),
        )

        show_export_files_dialog = action(
            get_str("exportFiles"),
            self.show_export_files_dir_dialog,
            None,
            "tools",
            get_str("exportFiles"),
        )
        ...
        self.menus = Struct(
            ...
            tools=self.menu(get_str("menu_tools")),
        ...
        add_actions(self.menus.tools, [show_rename_dialog, show_export_files_dialog])
    ...
    def update_all_label_combo_box(self):
        self.load_label_image_dict()
        # Get the unique labels and add them to the Combobox.

        unique_text_list = [key for key in self.label_image_dict.keys()]
        # Add a null row for showing all the labels
        unique_text_list.append("")
        unique_text_list.sort()

        self.all_label_combo_box.update_items(unique_text_list)
    ...
    def filter_images_by_label(self, index):
        self.load_label_image_dict()
        text = self.all_label_combo_box.cb.itemText(index)
        if text == "":
            self.m_img_list = self.m_img_list_all
        else:
            self.m_img_list = self.label_image_dict[text]
        ...
        if self.img_count > 0:
            ...
            self.cur_img_idx = 0
            index = self.m_img_list.index(self.m_img_list[0])
            file_widget_item = self.file_list_widget.item(index)
            file_widget_item.setSelected(True)
            # combobox_index = self.combo_box.cb.findText(text.split(":")[0])
            # self.combo_selection_changed(combobox_index)
        else:
            pass
    ...
    def import_dir_images(self, dir_path):
        ...
        self.m_img_list_all = self.m_img_list.copy()
        self.img_count = len(self.m_img_list)
        self.open_next_image()
        for imgPath in self.m_img_list:
            item = QListWidgetItem(imgPath)
    ...
    def load_label_image_dict(self):
        labels = []
        images = []
        verified_list = []
        for imgPath in self.m_img_list_all:
            tmp_shapes, tmp_verified = self.load_labels_from_annotation_file(imgPath)
            tmp_labels = [label for label, _, _, _, _ in tmp_shapes]
            tmp_images = [imgPath for i in range(len(tmp_labels))]
            labels += tmp_labels
            images += tmp_images
            verified_list.append(tmp_verified)

        self.label_image_dict = {}
        for label in list(set(labels)):
            tmp_list = list(
                sorted(
                    set([image for i, image in enumerate(images) if labels[i] == label])
                )
            )
            label = f"{label}: {len(tmp_list)}"
            self.label_image_dict[label] = tmp_list

        self.verified_image_list = []
        for i, verified in enumerate(verified_list):
            if verified is True:
                self.verified_image_list.append(self.m_img_list_all[i])

        # self.label_image_dict = {}
        # for label in list(set(labels)):
        #     label = f"label: {len(tmp_image_dict[label])}"
        #     self.label_image_dict[label] = list(
        #         set([image for i, image in enumerate(images) if labels[i] == label])
        #     )
        # print(self.label_image_dict)

    def show_rename_label_dialog(self):
        self.rename_label_dialog = RenameLabelDialog(parent=self)
        self.rename_label_dialog.show()

    def load_labels_from_annotation_file(self, file_path):
        shapes = []
        verified = False
        ...
        if self.default_save_dir is not None:
            ...
            if os.path.isfile(xml_path):
                ...
                t_voc_parse_reader = PascalVocReader(xml_path)
                shapes = t_voc_parse_reader.get_shapes()
                verified = t_voc_parse_reader.verified
            elif os.path.isfile(txt_path):
                self.set_format(FORMAT_YOLO)
                t_yolo_parse_reader = YoloReader(txt_path, self.image)
                shapes = t_yolo_parse_reader.get_shapes()
                verified = t_yolo_parse_reader.verified
            elif os.path.isfile(json_path):
                create_ml_parse_reader = CreateMLReader(json_path, file_path)
                shapes = create_ml_parse_reader.get_shapes()
                verified = create_ml_parse_reader.verified
        else:
            xml_path = os.path.splitext(file_path)[0] + XML_EXT
            txt_path = os.path.splitext(file_path)[0] + TXT_EXT
            if os.path.isfile(xml_path):
    ...
    def save_labels_to_annotation_file(self, shapes, annotation_file_path):
        image_file_name = os.path.basename(annotation_file_path)
        saved_file_name = os.path.splitext(image_file_name)[0]
    ...
    def export_labeled_files(self, dir):
        """"""
        self.load_label_image_dict()
    ...
    def show_export_files_dir_dialog(self):
        self.export_files_dir_dialog = ExportFilesDialog(parent=self)
        self.export_files_dir_dialog.show()
